...
<body>
    <div class="container">
        <h1>Bulkhead Pattern Demo</h1>
        <p style="text-align: center; color: #666; margin-bottom: 30px;">
            Sistema e-commerce con isolamento risorse e gestione priorità
        </p>

        <div class="grid">
            <!-- SERVICES -->
            <div class="section services-section">
                <h2>Servizi Isolati</h2>
                
                <h3>Processa Pagamento (Priorità Alta)</h3>
                <form id="paymentForm">
                ...
                <h3>Verifica Inventario (Priorità Media)</h3>
                ...
                <h3>Invia Notifica (Priorità Bassa)</h3>
                ...
                <h3>Genera Report (Priorità Bassa)</h3>
                <form id="reportForm">
                    <div class="form-group">
                        <label>Type:</label>
                        <select name="type">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="parameter" value="monthly" required>
                    </div>
                    <button type="submit" class="service-btn">Genera Report</button>
                </form>
            </div>
        ...
        <div class="section management-section full-width">
            <h2>Gestione Bulkhead</h2>
            
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                <button onclick="resetBulkhead('payment')" class="management-btn">Reset Payment</button>
                <button onclick="resetBulkhead('inventory')" class="management-btn">Reset Inventory</button>
                <button onclick="resetBulkhead('notification')" class="management-btn">Reset Notification</button>
                <button onclick="resetBulkhead('report')" class="management-btn">Reset Report</button>
                <button onclick="resetAllBulkheads()" class="management-btn">Reset All</button>
            </div>
        </div>
    ...
    <script>
        ...
        document.getElementById('paymentForm').addEventListener('submit', async (e) => {
            ...
            await makeRequest('/bulkhead/payment', 'POST', data);
        });
        ...
        document.getElementById('inventoryForm').addEventListener('submit', async (e) => {
            ...
            await makeRequest('/bulkhead/inventory', 'POST', data);
        });
        ...
        document.getElementById('notificationForm').addEventListener('submit', async (e) => {
            ...
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData.entries());
            await makeRequest('/bulkhead/notification', 'POST', data);
        });
        ...
        document.getElementById('reportForm').addEventListener('submit', async (e) => {
            ...
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData.entries());
            await makeRequest('/bulkhead/report', 'POST', data);
        });

        async function getAllServicesStatus() {
            const response = await fetch('/bulkhead/status');
            const result = await response.json();
        ...
        async function getMetrics() {
            const response = await fetch('/bulkhead/metrics');
            const result = await response.json();
        ...
        async function resetBulkhead(serviceName) {
            await makeRequest(`/bulkhead/reset/${serviceName}`, 'POST');
        }

        async function resetAllBulkheads() {
            await makeRequest('/bulkhead/reset-all', 'POST');
        }
            ...
            Object.entries(services).forEach(([serviceName, status]) => {
                ...
                serviceCard.innerHTML = `
                    ...
                    <div style="font-size: 12px; color: #666;">
                        <div>Active Threads: ${status.active_threads || 0} / ${status.config?.max_threads || 0}</div>
                        <div>Active Connections: ${status.active_connections || 0} / ${status.config?.max_connections || 0}</div>
                        <div>Queue Length: ${status.queue_length || 0}</div>
        ...
        function displayMetrics(metrics) {
            ...
            Object.entries(metrics).forEach(([serviceName, metric]) => {
                ...
                metricCard.innerHTML = `
                    ...
                    <div class="metric-value">${metric.total_executions || 0}</div>
                    <div class="metric-label">Total Executions</div>
                    <div class="metric-value">${metric.successful_executions || 0}</div>
                    <div class="metric-label">Successful</div>
                    <div class="metric-value">${metric.failed_executions || 0}</div>
                    <div class="metric-label">Failed</div>
                    <div class="metric-value">${metric.avg_execution_time?.toFixed(3) || 0}s</div>
