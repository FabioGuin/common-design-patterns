...
class InventoryService
{
    ...
    {
        return $this->retryManager->execute('inventory_service', function () use ($productId, $quantity) {
    ...
    private function callExternalInventoryService(string $productId, int $quantity): array
    {
        ...
        if (rand(1, 4) === 1) {
            throw new \Exception("Inventory service temporarily unavailable", 503);
        }
    ...
    private function callExternalReserveService(string $productId, int $quantity): array
    {
        ...
        // Simula fallimento casuale per testing
        if (rand(1, 5) === 1) {
            throw new \Exception("Reservation service temporarily unavailable", 502);
        }
    ...
    private function callExternalUpdateService(string $productId, int $quantity): array
    {
        ...
        // Simula fallimento casuale per testing
        if (rand(1, 6) === 1) {
            throw new \Exception("Update service temporarily unavailable", 500);
        }
    ...
    {
        return $this->retryManager->getRetryStatus('inventory_service') ?? [
