...
<head>
    ...
    <title>Timeout Pattern Demo</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Timeout Pattern Demo</h1>
        <p style="text-align: center; color: #666; margin-bottom: 30px;">
            Sistema e-commerce con timeout dinamici e gestione operazioni lente
        </p>

        <div class="grid">
            <!-- SERVICES -->
            <div class="section services-section">
                <h2>Servizi con Timeout</h2>
                
                <h3>Processa Pagamento (Timeout 15s)</h3>
                <form id="paymentForm">
                ...
                <h3>Verifica Inventario (Timeout 10s)</h3>
                ...
                <h3>Invia Notifica (Timeout 5s)</h3>
    ...
    <script>
        ...
        document.getElementById('paymentForm').addEventListener('submit', async (e) => {
            ...
            await makeRequest('/timeout/payment', 'POST', data);
        });
        ...
        document.getElementById('inventoryForm').addEventListener('submit', async (e) => {
            ...
            await makeRequest('/timeout/inventory', 'POST', data);
        });
        ...
        document.getElementById('notificationForm').addEventListener('submit', async (e) => {
            ...
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData.entries());
            await makeRequest('/timeout/notification', 'POST', data);
        });

        async function getAllServicesStatus() {
            const response = await fetch('/timeout/status');
            const result = await response.json();
        ...
        async function getMetrics() {
            const response = await fetch('/timeout/metrics');
            const result = await response.json();
        ...
        async function getTimeoutEvents() {
            const response = await fetch('/timeout/events');
            const result = await response.json();
            
            if (result.success) {
                displayTimeoutEvents(result.events);
            } else {
                showResult(result, true);
            }
        }
            ...
            Object.entries(services).forEach(([serviceName, status]) => {
                ...
                serviceCard.innerHTML = `
                    ...
                    <div style="font-size: 12px; color: #666;">
                        <div>Timeout: <span class="timeout-badge">${status.config?.timeout || 0}ms</span></div>
                        <div>Max Total: ${status.config?.max_total_timeout || 0}ms</div>
                        <div>Max Attempts: ${status.config?.max_attempts || 0}</div>
                        <div>Success Rate: ${status.success_rate?.toFixed(1) || 0}%</div>
                        <div>Timeout Rate: ${status.timeout_rate?.toFixed(1) || 0}%</div>
                        <div>Avg Time: ${status.avg_execution_time?.toFixed(3) || 0}s</div>
        ...
        function displayMetrics(metrics) {
            ...
            Object.entries(metrics).forEach(([serviceName, metric]) => {
                ...
                metricCard.innerHTML = `
                    ...
                    <div class="metric-value">${metric.total_operations || 0}</div>
                    <div class="metric-label">Total Operations</div>
                    <div class="metric-value">${metric.successful_operations || 0}</div>
                    <div class="metric-label">Successful</div>
                    <div class="metric-value">${metric.timeout_operations || 0}</div>
        ...
        function displayTimeoutEvents(events) {
            ...
            Object.entries(events).forEach(([serviceName, event]) => {
                ...
                eventCard.innerHTML = `
                    ...
                    <div class="metric-value">${event.timeout_events || 0}</div>
                    <div class="metric-label">Timeout Events</div>
                    <div class="metric-value">${event.error_events || 0}</div>
                    <div class="metric-label">Error Events</div>
                    <div class="metric-value">${event.avg_execution_time?.toFixed(3) || 0}s</div>
