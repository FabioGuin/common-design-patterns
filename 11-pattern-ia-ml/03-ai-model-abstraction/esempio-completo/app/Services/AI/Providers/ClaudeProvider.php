...
class ClaudeProvider implements AIModelInterface
{
    private string $name;
    ...
    {
        return $this->modelConfig['description'] ?? 'Claude Model';
    }
    ...
    public function validateOptions(array $options): array
    {
        ...
        if (isset($options['temperature'])) {
            $validated['temperature'] = max(0, min(1, $options['temperature']));
        }
    ...
    public function getLimits(): array
    {
        return [
            'max_tokens' => $this->getMaxTokens(),
            'context_window' => $this->getContextWindow(),
            'max_requests_per_minute' => $this->modelConfig['max_requests_per_minute'] ?? 50,
            'cost_per_token' => $this->getCostPerToken()
        ];
    }
    ...
    {
        $providerConfig = config('ai_models.providers.claude');
    ...
    /**
     * Effettua una richiesta all'API Claude
     */
    private function makeRequest(string $endpoint, array $data): array
    {
        ...
        ->post($providerConfig['base_url'] . '/' . $endpoint, $data);
